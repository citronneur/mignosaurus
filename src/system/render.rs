...
impl Sprite {
    ...
    /// # }
    /// ```
    pub fn new(sprite_reference: SpriteRef, z: i16) -> Self {
        Self {
...
impl<'a> System<'a> for RenderSystem {
    ...
    fn run(&mut self, (mut buffer, sprites): Self::SystemData) {
        ...
        let mut ordered_sprite = vec![];
        for sprite_component in sprites.join() {
            ordered_sprite.push(sprite_component);
        }

        ordered_sprite.sort_by(|a, b| {
            a.z.cmp(&b.z)
        });

        for sprite_component in ordered_sprite {
